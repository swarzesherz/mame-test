...
void neogeo_bootleg_sx_decrypt( running_machine *machine, int value )
{
	...
	if (value == 1)
	{
		UINT8 *buf = malloc_or_die( sx_size );
...
void install_fr2ch_fix( running_machine *machine )
{
	...
	*((unsigned short*)(memory_region( machine, "maincpu" ) + 0x1BF2)) = 0x4E71;
	*((unsigned short*)(memory_region( machine, "maincpu" ) + 0x1BF4)) = 0x4E71;
	*((unsigned short*)(memory_region( machine, "maincpu" ) + 0x1BF6)) = 0x4E71;
}

//void neogeo_bootleg_text_decrypt(INT32 value)
void neogeo_bootleg_text_decrypt(running_machine *machine, int value)
{
	INT32 i, ofst, size = memory_region_length( machine, "fixed" );
	UINT8 *srom = memory_region( machine, "fixed" );
...
{
	if (offset == 1) {
		//neogeo_set_main_cpu_bank_address(space->machine, data << 8 );	
		neogeo_set_main_cpu_bank_address(space, data << 8 );			
	}
}

void ms5boot_install_protection( running_machine *machine )
{
	// special ROM banking handler
	memory_install_write16_handler(cpu_get_address_space(machine->cpu[0], ADDRESS_SPACE_PROGRAM), 0x2ffff0, 0x2fffff, 0, 0, mslug5_bankswitch_w);
}